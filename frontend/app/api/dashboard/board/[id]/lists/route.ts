...
export async function POST(req: Request, context: { params: { id: string } }) {
  ...
  const maxPos = await prisma.list.aggregate({ where: { board_id: boardId }, _max: { position: true } })
  const position = (maxPos._max.position ?? -1) + 1

  const list = await prisma.list.create({ data: { board_id: boardId, title, position } })
